...
import {
  appointmentClaimedMetaKeys,
  extractLeadAppointments,
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-lg border border-border/50 bg-card overflow-hidden">
        {appointments.length > 0 && (
          <div className="border-b border-border/40 bg-muted/30 px-3 py-2.5 space-y-2.5">
            {appointments.map((appointment) => {
              const past = isAppointmentPast(appointment);
              return (
                <div key={appointment.key} className="flex items-start gap-2.5">
                  <TooltipContainer
                    tooltipContent={t(
                      `leads.appointment.provider.${appointment.provider}`,
                      { defaultValue: "Calendar" },
                    )}
                    showCopyButton={false}
                  >
                    <span className="mt-0.5 inline-flex h-4 w-4 shrink-0 items-center justify-center">
                      <AppointmentProviderIcon
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-[11px] font-medium text-muted-foreground/80">
                      {t("leads.appointment.title", {
                        defaultValue: "Appointment",
                      })}
                      {past
                        ? ` · ${t("leads.appointment.past", { defaultValue: "Past" })}`
                        : null}
                    </p>
                    <p
                      className={cn(
                    ...
                    {!appointment.booked && (
                      <p className="text-[11px] text-amber-600 dark:text-amber-500">
                        {t("leads.appointment.notBooked", {
                          defaultValue: "Not confirmed in calendar",
                        })}
                      </p>
                    )}
                  </div>
